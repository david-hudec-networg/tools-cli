...
/// Usage: <c>txc security role list [--filter &lt;name&gt;]</c>
/// </summary>
[CliReadOnly]
[CliCommand(
    Name = "list",
    Description = "List tenant roles accepted by --role in txc security service-principal/user/group role add/remove commands."
)]
public class RoleListCliCommand : ProfiledCliCommand
{
    ...
    {
        var roles = await SecurityRoleCommandSupport.ListRolesAsync(Profile, Filter, CancellationToken.None).ConfigureAwait(false);
