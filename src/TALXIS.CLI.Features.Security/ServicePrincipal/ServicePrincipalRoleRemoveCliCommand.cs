...
/// Usage: <c>txc security service-principal role remove --service-principal &lt;client-id-or-object-id&gt; --role &lt;name-or-guid&gt; --yes</c>
/// </summary>
...
public class ServicePrincipalRoleRemoveCliCommand : ProfiledCliCommand, IDestructiveCommand
{
    ...
    private async Task<int> ExecuteRemoveRoleAsync()
    {
        try
        {
            await SecurityServicePrincipalCommandSupport.RemoveAssignmentAsync(Profile, ServicePrincipal, Role, CancellationToken.None).ConfigureAwait(false);
            ...
            SecurityServicePrincipalCommandSupport.WriteMutationResult(payload, () =>
            {
