...
/// Usage: <c>txc security service-principal role add --service-principal &lt;client-id-or-object-id&gt; --role &lt;name-or-guid&gt;</c>
/// </summary>
[CliIdempotent]
[CliCommand(
    Name = "add",
...
public class ServicePrincipalRoleAddCliCommand : ProfiledCliCommand
{
    ...
    private async Task<int> ExecuteAddRoleAsync()
    {
        try
        {
            await SecurityServicePrincipalCommandSupport.AddAssignmentAsync(Profile, ServicePrincipal, Role, CancellationToken.None).ConfigureAwait(false);
