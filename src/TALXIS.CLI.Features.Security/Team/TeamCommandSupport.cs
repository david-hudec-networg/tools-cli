...
#pragma warning disable TXC003
        ...
        foreach (var row in rows)
        {
            OutputWriter.WriteLine(
                ...
                $"{SecurityPrincipalCommandSupport.Truncate(ToCliValue(row.TeamType), typeWidth).PadRight(typeWidth)} | " +
                $"{SecurityPrincipalCommandSupport.Truncate(ToCliValue(row.MembershipType) ?? "-", membershipWidth).PadRight(membershipWidth)} | " +
...
#pragma warning disable TXC003
        ...
        {
            OutputWriter.WriteLine(
                $"{SecurityPrincipalCommandSupport.Truncate(row.FullName ?? "-", nameWidth).PadRight(nameWidth)} | " +
                $"{SecurityPrincipalCommandSupport.Truncate(row.UserPrincipalName ?? "-", upnWidth).PadRight(upnWidth)} | " +
                $"{SecurityPrincipalCommandSupport.Truncate(row.PrimaryEmailAddress ?? "-", emailWidth).PadRight(emailWidth)} | " +
                $"{SecurityPrincipalCommandSupport.Truncate(row.BusinessUnitName ?? "-", businessUnitWidth).PadRight(businessUnitWidth)} | " +
